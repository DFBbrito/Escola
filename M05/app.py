...
def adicionar_livro(livros,nr_livros):
    if nr_livros==len(livros):
        print("Não é possivel adicionar mais livros.")
        return
...
def emprestar_livro(livros,nr_livros):
    ...
    #verificar se o id exite e se o livro esta disponivel
    for i in range(nr_livros):
...
def remover_livro(livros,nr_livros):
    pass

#Função que apresenta as opções ao utilizador e devolve a sua escolha
def menu():
    while True:
        print("\n!MENU\n1.Adicionar livro\n2.Ver livros\n3.Emprestar livro\n4.Devoluçao do livro\n5.Remover livro\n6.Sair")
        op=int(input(":"))
        if op <1 or op>6:
            print("Essa opçao não é valida")
        else:
            return op
...
def main():
    ...
    while op!=6:
        ...
        if op==1:
            ...
            remover_livro(livros,nr_livros)
